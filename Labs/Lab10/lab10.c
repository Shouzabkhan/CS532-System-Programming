...
int main(int argc, char *argv[]) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (pid < 0) {
            ...
            int fdout = open(out_filename, O_CREAT | O_WRONLY | O_TRUNC, 0644);
            int fderr = open(err_filename, O_CREAT | O_WRONLY | O_TRUNC, 0644);
            ...
            dup2(fdout, STDOUT_FILENO);  // Redirect stdout to <pid>.out
            dup2(fderr, STDERR_FILENO);  // Redirect stderr to <pid>.err
