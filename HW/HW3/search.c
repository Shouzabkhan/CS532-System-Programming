...
void handle_file(const char *dirpath, const struct dirent *entry, const struct stat *st, int depth, options_t *opts) {
    if ((opts->pattern == NULL || strstr(entry->d_name, opts->pattern)) &&
        ...
        // Execute the -e command if specified
        ...
        // Execute the -E command if specified
        if (opts->exec_command_E) {
            pid_t pid = fork();
            if (pid == 0) { // Child process
                char cmd[PATH_MAX];
                snprintf(cmd, sizeof(cmd), "%s %s", opts->exec_command_E, entry->d_name);
                execlp("/bin/sh", "sh", "-c", cmd, (char *)NULL);
                perror("execlp"); // Handle exec error
                exit(EXIT_FAILURE);
            } else if (pid > 0) {
...
void usage(const char *progname) {
    fprintf(stderr, "Usage: %s [-S] [-s size] [-f pattern depth] [-t {f|d}] [-e command] [-E command] [directory]\n", progname);
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {
        ...
        {"execute", required_argument, 0, 'e'},     // Option for -e
        {"execute-all", required_argument, 0, 'E'}, // Option for -E
        {0, 0, 0, 0}
    };
...
void handle_file(const char *dirpath, const struct dirent *entry, const struct stat *st, int depth, options_t *opts) {
    if ((opts->pattern == NULL || strstr(entry->d_name, opts->pattern)) &&
        ...
        if (opts->exec_command_E) {
            ...
            if (pid == 0) { // Child process
                char cmd[PATH_MAX];
                snprintf(cmd, sizeof(cmd), "%s %s/%s", opts->exec_command_E, dirpath, entry->d_name);
                printf("Executing command: %s\n", cmd); // Debugging line
                execlp("/bin/sh", "sh", "-c", cmd, (char *)NULL);
                perror("execlp"); // Handle exec error
                exit(EXIT_FAILURE);
